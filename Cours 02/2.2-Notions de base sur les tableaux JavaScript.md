# Notions de base sur les tableaux JavaScript (Arrays)

Les tableaux (arrays) sont des collections ordonnées de valeurs (de n'importe quel type : nombres, chaînes, objets, fonctions…). Ils sont indexés à partir de 0.

## 1. Définition et syntaxe

```javascript
// Littéral de tableau (recommandé)
const notes = [12, 14, 9, 16];

// Tableau hétérogène
const mix = [42, "allo", true, { x: 1 }, [1, 2]];

// Tableau vide, puis remplissage
let fruits = [];
fruits = ["pomme", "banane"];

// Constructeur (à éviter pour créer un tableau rempli d'éléments identiques)
const arr1 = new Array(3);      // [ <3 éléments vides> ] -> longueur 3, valeurs "vides"
const arr2 = Array.of(3);       // [3] -> utilise Array.of pour éviter l'ambiguïté
```

✅ Conseil : Préférez la notation littérale `[ ]` et `Array.of()` lorsque nécessaire.

## 2. Taille : propriété `.length`

La propriété `length` donne le nombre d'éléments dans le tableau. Il est mutable : le réduire tronque le tableau. L'augmenter va ajouter des éléments vides (*empty items*).

```javascript
const a = ["A", "B", "C"];
console.log(a.length); // 3

a.length = 2;          // tronque
console.log(a);        // ["A", "B"]

a.length = 5;          // étend -> ajoute des "trous" (éléments vides)
console.log(a);        // ["A", "B", <3 empty items>]
```

## 3. Accéder à un élément (lire) / Remplacer un élément : indices

On utilise les indices positifs, qui vont de `0` (premier) à `length - 1` (dernier).

```javascript
const t = ["A", "B", "C", "D", "E"];
// Lire via les indices
console.log(t[0]);  // "A"
console.log(t[t.length - 1]);  // "E", équivaut à t[4]
console.log(t[99]); // undefined (hors bornes), n'est pas une valeur vide

// Remplacer un élément
t[1] = "Z";
console.log(t); // [ 'A', 'Z', 'C', 'D', 'E' ]
// On peut aussi ajouter au-delà de la fin (créera des "trous") :
const t2 = ["A"];
t2[3] = "X";
console.log(t2); // ["A", <2 empty items>, "X"]
```

## 4. Parcourir un tableau

### Boucle `for` standard

✅ Contrôle total sur l'indice.

```javascript
const tFor = [10, 20, 30];
for (let i = 0; i < tFor.length; i++) {
  console.log(i, tFor[i]); // indice, valeur
}
```

### Boucle `for of`

✅ Lisible, moderne.  
❌ N'offre pas directement l'indice (utiliser `t.entries()` si besoin).

```javascript
const tOf = ["a", "b", "c", "d", "e"];
for (const valeur of tOf) {
  console.log(valeur);
}
// Pour accéder à l'indice, utiliser .entries()
for (const [i, v] of tOf.entries()) {
  console.log(i, v);
}
```

## 5. Méthodes utiles

### [`at(index)`](https://developer.mozilla.org/fr/docs/Web/JavaScript/Reference/Global_Objects/Array/at)

- **Effet** : Immuable (n'altère pas le tableau)
- **But** : accès par indice **positif** ou **négatif** (depuis la fin)
- **Retour** : la valeur à l'indice donné ou `undefined`

```javascript
const t = ["A", "B", "C"];
t.at(0);  // "A"
t.at(-1); // "C"
t.at(3); // undefined
t.at(-3); // "A"
t.at(-4); // undefined
```

### [`push(...items)`](https://developer.mozilla.org/fr/docs/Web/JavaScript/Reference/Global_Objects/Array/push)

- **Effet** : Muable (ajoute en **fin**)
- **But** : Ajouter un élément
- **Retour** : La nouvelle longueur du tableau

```javascript
const tPush = [1, 2];
const n = tPush.push(3, 4);
console.log(tPush, n); // [ 1, 2, 3, 4 ] 4
```

### [`pop()`](https://developer.mozilla.org/fr/docs/Web/JavaScript/Reference/Global_Objects/Array/pop)

- **Effet** : Muable (retire le **dernier**)
- **But** : Retirer le dernier élément
- **Retour** : Le dernier élément, celui qui est retiré. `undefined` si le tableau est vide.

```javascript
const tPop = [8, 9, 10, 11];
const r = tPop.pop();
console.log(tPop, r); // [ 8, 9, 10 ] 11
```

### [`unshift(...items)`](https://developer.mozilla.org/fr/docs/Web/JavaScript/Reference/Global_Objects/Array/unshift)

- **Effet** : Muable (aujoute en **début**)
- **But** : Ajouter un élément
- **Retour** : La nouvelle longueur du tableau

```javascript
const tUnshift = [9, 12, 15];
const m = tUnshift.unshift(3, 6);
console.log(tUnshift, m); // [ 3, 6, 9, 12, 15 ] 5
```

### [`shift()`](https://developer.mozilla.org/fr/docs/Web/JavaScript/Reference/Global_Objects/Array/shift)

- **Effet** : Muable (retire le **premier**)
- **But** : Retrait du premier élément
- **Retour** : L'élément retiré (ou `undefined` si vide)

```javascript
const tShift = [4, 5, 6];
const d = tShift.shift();
console.log(tShift, d); // [ 5, 6 ] 4
```

### [`indexOf(value, fromIndex = 0)`](https://developer.mozilla.org/fr/docs/Web/JavaScript/Reference/Global_Objects/Array/indexOf)

- **Effet** : Immuable
- **But** : Recherche par égalité stricte (`===`) [pour en savoir plus](https://developer.mozilla.org/fr/docs/Web/JavaScript/Reference/Operators/Strict_equality)
- **Retour** : Le premier indice de `value`, ou `-1` si non trouvé

💡 Pour objets/références, `indexOf` ne fonctionne que si c'est la **même référence**.

```javascript
const unTab = ['x', 'y', 'z'];
const tIndexOf = ["A", "B", "A"];
console.log(tIndexOf.indexOf("A"));    // 0
console.log(tIndexOf.indexOf("A", 1)); // 2
console.log(tIndexOf.indexOf("Z"));    // -1
tIndexOf.push(['x', 'y', 'z']);        // pas la même référence que unTab
console.log(tIndexOf.indexOf(unTab));  // -1
console.log(tIndexOf.indexOf(['x', 'y', 'z']));  // -1
```

### [`sort(compareFn?)`](https://developer.mozilla.org/fr/docs/Web/JavaScript/Reference/Global_Objects/Array/sort)

- **Effet** : Muable, peut déplacer les éléments
- **But** : Trier les éléments. Sans `compareFn`, va faire un tri alphabétique (ordre des caractère `UTF-16`)
- **Retour** : La référence vers le tableau d'origine, désormais trié.

✅ Astuce : utiliser une copie pour conserver l'original

```javascript
const tSort1 = [10, 2, 30];
console.log(tSort1.sort()); // ["10", "2", "30"] -> tri lexicographique -> [10, 2, 30] "mal trié" numériquement
const tSort2 = [10, 2, 30];
console.log(tSort2.sort((a, b) => a - b)); // [2, 10, 30]
const tSort3 = ["A", "a", "B"];
const tSort3Trie = [...tSort3].sort(); // 
const tSort3Trie2 = tSort3.slice().sort(); // .slice() retourne une copie superficielle
console.log(tSort3, tSort3Trie, tSort3Trie2); // [ 'A', 'a', 'B' ] [ 'A', 'B', 'a' ] [ 'A', 'B', 'a' ]
// Les majuscules sont avant les minuscule en ordre UTF-16
```

### [`reverse()`](https://developer.mozilla.org/fr/docs/Web/JavaScript/Reference/Global_Objects/Array/reverse)

- **Effet** : Muable
- **But** : Renverser l'ordre **actuel** des éléments (pas ordre inversé!)
- **Retour** : La référence vers le tableau d'origine, désormais inversé.

✅ Immuable alternatif : `const r = [...t].reverse();`

```javascript
const tReverse1 = [3, 1, 2, 5, 4];
console.log(tReverse1.reverse()); // [ 4, 5, 2, 1, 3 ]
const tReverse2 = [8, 6, 3, 1];
const tReverse2Inverse = [...tReverse2].reverse();
console.log(tReverse2, tReverse2Inverse); // [ 8, 6, 3, 1 ] [ 1, 3, 6, 8 ]
```

### [`slice(start?, end?)`](https://developer.mozilla.org/fr/docs/Web/JavaScript/Reference/Global_Objects/Array/slice)

- **Effet** : Immuable
- **But** : Extrait une copie (superficielle) d'une portion `[start, end)`
- **Retour** : Un **nouveau tableau** contenant les éléments extraits.

```javascript
console.log(tSlice.slice(1, 3));  // ["B", "C"]
console.log(tSlice.slice(2));     // ["C", "D"]
console.log(tSlice.slice(-2));    // ["C", "D"]
console.log(tSlice.slice(4));     // []
const copieSlice = tSlice.slice(); // copie superficielle
console.log(tSlice, copieSlice.reverse()); // [ 'A', 'B', 'C', 'D' ] [ 'D', 'C', 'B', 'A' ] // original pas affecté! 
```

### [`splice(start, deleteCount?, ...itemsToAdd)`](https://developer.mozilla.org/fr/docs/Web/JavaScript/Reference/Global_Objects/Array/splice)

- **Effet** : Muable
- **But** : **Enlever** et/ou **insérer** des éléments au milieu
- **Retour** : Un tableau contenant les éléments supprimés. Si un seul élément est supprimé, un tableau contenant cet unique élément est retourné. Si aucun élément n'est supprimé, un tableau vide est retourné.

```javascript
const tSplice = ["A", "B", "C", "D"];
// Suppression
const suppr = tSplice.splice(1, 2);
console.log(tSplice, suppr); // [ 'A', 'D' ] [ 'B', 'C' ]
// Insertion
const suppr2 = tSplice.splice(1, 0, "X", "Y");
console.log(tSplice, suppr2); // [ 'A', 'X', 'Y', 'D' ] []
// Remplacement
const suppr3 = tSplice.splice(1, 2, "Q");
console.log(tSplice, suppr3); // [ 'A', 'Q', 'D' ] [ 'X', 'Y' ]
```

### [`toString()`](https://developer.mozilla.org/fr/docs/Web/JavaScript/Reference/Global_Objects/Array/toString)

- **Effet** : Immuable
- **But** : Avoir une représentation du tableau en `string`
- **Retour** : Renvoie une chaîne des éléments séparés par des virgules.

```javascript
const tToString = [1, "A", true];
console.log(tToString.toString()); // "1,A,true"
```

## 6. Notes & Bonnes pratiques

- **Mutation vs immutabilité** : privilégiez les méthodes **immuables** (`slice`, `at`, `indexOf`, `toString`, `join`) si vous voulez éviter les effets de bord. Les méthodes **mutables** (`push`, `pop`, `shift`, `unshift`, `sort`, `reverse`, `splice`) modifient le tableau d'origine.
- **Copies rapides** : `const copie = [...t]` (spread) ou `t.slice()` (copie superficielle).
- **Tri numérique** : toujours fournir un comparateur `sort((a, b) => a - b)`.
- **Indices négatifs** : `at(-1)` (préférable à `t[t.length - 1]` pour la lisibilité).
- **Performance** : `push`/`pop` sont généralement plus performants que `shift`/`unshift` (car ces derniers re-indexent les éléments).
