...
- La **fonction de rappel peut prendre 4 paramètres** :
    1. `accumulateur` : La valeur retournée quand tous les éléments auront été traités.
    ...
    4. `tableau` : Le tableau parcouru, sur lequel `.reduce()` est appelé.
...
console.log(memeSomme) // 10
// équivalent à 
// memeSomme = [2, 3, 4].reduce((acc, elm) => acc + elm, 1);
```
